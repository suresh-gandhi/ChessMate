		...
		if (isWhite == Board.IsWhiteToPlay()) {
			...
			for (int i = 0; i < legalMoves.Count; i ++) {
				...
				if (moveCoords == algebraicMove) { // move confirmed as legal
                    ...
                    Debug.Log("The move played is : " + algebraicMove);
                    // Here we will add Gamesparks unit to send this to server as a move
                    // And then make the whole board system uninteractable i.e. ChessInput uninteractable
					break;
				}
			}
		}
