...
public static class ZobristKeyGround {
	...
	public static ulong GetZobristKey() {
		...
		for (int squareIndex = 0; squareIndex < 120; squareIndex ++) {
			if (BoardGround.boardColourArray[squareIndex] != -1) {
				zobristKey ^= piecesArray[((BoardGround.boardArray[squareIndex] & ~1) >> 1) -1,BoardGround.boardColourArray[squareIndex],squareIndex];
			}
		}

		int epIndex = (BoardGround.currentGamestate >> 5 & 15) -1;
		if (epIndex != -1) {
			zobristKey ^= enPassantSquare[epIndex + (((BoardGround.currentGamestate & 1) == 1)?80:32)];
		}

		if ((BoardGround.currentGamestate & 1) == 0) {
			zobristKey ^= sideToMove;
		}

		zobristKey ^= castlingRightsWhite[(BoardGround.currentGamestate >> 1) & 3];
		zobristKey ^= castlingRightsBlack[(BoardGround.currentGamestate >> 3) & 3];
