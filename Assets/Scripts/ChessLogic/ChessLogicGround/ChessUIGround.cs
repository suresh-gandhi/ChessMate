...
public class ChessUIGround : MonoBehaviour
{
    public Material whiteMaterial;
    public Material blackMaterial;

    public Color highlightedColor;
    public Color moveFromColor;
    ...
    static ChessUIGround myInstance;
    ...
    */
    ...
    /// Gets the instance.
    /// </summary>
    public static ChessUIGround instance
    {
        get
        {
            if (myInstance == null)
            {
                myInstance = FindObjectOfType<ChessUIGround>();
            }
            return myInstance;
        }
    }
    ...
    public void HighlightSquare(string squaresToHighlight)
    {
        if (highlightLegalMoves)
        {
            ...
            cubes[squareX, squareY].material.color = highlightedColor;
    ...
    public void HighlightMove(int fromIndex, int toIndex)
    {
        ...
        cubes[fromX, fromY].material.color = moveFromColor;
        cubes[toX, toY].material.color = moveToColor;
    ...
    public void CreateBoardUI()
    {
        ...
        boardHolder.localRotation = Quaternion.identity;
        ...
        pieceHolder.localRotation = Quaternion.identity;
        ...
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                ...
                Vector3 localPosition = new Vector3(-11.5f + x * 3 + 1, 0, -11.5f + y * 3 + 1);
                ...
                // squares
                Transform newSquare = Instantiate((isLightSquare) ? lightSquare : darkSquare, Vector3.zero, Quaternion.identity) as Transform;
                newSquare.parent = boardHolder;
                newSquare.name = algebraicCoordinate;
                newSquare.localPosition = localPosition;        // added line.
                newSquare.localRotation = Quaternion.identity;
