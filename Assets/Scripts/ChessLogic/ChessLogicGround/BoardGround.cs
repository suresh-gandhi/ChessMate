...
public static class BoardGround {
	...
	/// BoardGround array of piece codes.
	/// Note that colour information is included in the codes.
	public static int[] boardArray;
	/// BoardGround array representing colour of pieces. (0=black, 1=white, -1=vacant)
	...
	public static void UnmakeMove (ushort move, bool updateUI = false)
	{
		...
		// Update zobrist key
		zobristKey ^= ZobristKeyGround.sideToMove; // toggle side to move
		zobristKey ^= ZobristKeyGround.piecesArray[(movePieceType >> 1) - 1, colourToMove,moveToIndex]; // remove piece from old square
		zobristKey ^= ZobristKeyGround.piecesArray[(movePieceType >> 1) - 1, colourToMove,moveFromIndex]; // place piece at new square
		if (capturedPieceCode != 0) {
			zobristKey ^= ZobristKeyGround.piecesArray [((capturedPieceCode & ~1) >> 1) - 1, 1 - colourToMove, moveToIndex]; // remove captured piece
		}

		// remove old ep square from zobrist key
		int epIndex = (gamestateBeforeUndo >> 5 & 15) -1;
		if (epIndex != -1) {
			zobristKey ^= ZobristKeyGround.enPassantSquare[epIndex + (((currentGamestate & 1) == 0)?80:32)];
		}

		// add new ep square to zobrist key
		epIndex = (currentGamestate >> 5 & 15) -1;
		if (epIndex != -1) {
			zobristKey ^= ZobristKeyGround.enPassantSquare[epIndex + (((currentGamestate & 1) == 1)?80:32)];
		}

		// remove old castling rights from zobrist key
		zobristKey ^= ZobristKeyGround.castlingRightsWhite[(gamestateBeforeUndo >> 1) & 3];
		zobristKey ^= ZobristKeyGround.castlingRightsBlack[(gamestateBeforeUndo >> 3) & 3];
		
		// add new castling rights to zobrist key
		zobristKey ^= ZobristKeyGround.castlingRightsWhite[(currentGamestate >> 1) & 3];
		zobristKey ^= ZobristKeyGround.castlingRightsBlack[(currentGamestate >> 3) & 3];
		...
		if ((gamestateBeforeUndo & 1 << 15) != 0) { // move was castles; move rook back to original square
			...
			zobristKey ^= ZobristKeyGround.piecesArray[(rookCode>>1) - 1, colourToMove, rookFromIndex]; // remove rook from old square in zobrist key
			zobristKey ^= ZobristKeyGround.piecesArray[(rookCode>>1) - 1, colourToMove, rookToIndex]; // add rook to new square in zobrist key
		}
			...
			zobristKey ^= ZobristKeyGround.piecesArray[(pawnCode>>1)-1, 1-colourToMove, epCapturedPawnIndex]; // add captured ep pawn to zobrist key
		}
			...
			zobristKey ^= ZobristKeyGround.piecesArray[(movePieceType >> 1) - 1, colourToMove,moveFromIndex]; // remove promoted piece from zobrist key
			zobristKey ^= ZobristKeyGround.piecesArray[(pawnCode>>1)-1,colourToMove, moveFromIndex]; // add the demoted pawn to zobrist key
		}
	...
	public static void MakeMove (ushort move, bool updateUI = false)
	{
		...
		// Update zobrist key
		zobristKey ^= ZobristKeyGround.sideToMove; // toggle side to move
		zobristKey ^= ZobristKeyGround.piecesArray[(movePieceType >> 1) - 1, colourToMove,moveToIndex]; // place piece at new square
		zobristKey ^= ZobristKeyGround.piecesArray[(movePieceType >> 1) - 1, colourToMove,moveFromIndex]; // remove piece from old square
		...
		if (movePieceType == pawnCode) {
			if (moveToIndex >= 112 || moveToIndex <= 7) { // pawn has reached first/eighth rank
				...
				zobristKey ^= ZobristKeyGround.piecesArray[(movePieceType >> 1)-1, colourToMove,moveToIndex]; // remove pawn from zobrist key
				zobristKey ^= ZobristKeyGround.piecesArray[(pieceCodeArray [promotionPieceIndex] >> 1)-1, colourToMove,moveToIndex]; // add promoted piece to zobrist key
			}
				...
				if (capturedPieceCode == 0) { // seemingly capturing empty square, thus en passant capture has occurred
					...
					zobristKey ^= ZobristKeyGround.piecesArray[(pawnCode >> 1)-1, 1-colourToMove,epCapturedPawnIndex]; // remove ep captured pawn from zobrist key
				}
			}
		} 
		// Castling
		...
		// add new castling rights to zobrist key
		zobristKey ^= ZobristKeyGround.castlingRightsWhite[(newGamestate >> 1) & 3];
		zobristKey ^= ZobristKeyGround.castlingRightsBlack[(newGamestate >> 3) & 3];
	...
	static void UpdatePhysicalBoardWithMoveHighlights(int moveFromIndex, int moveToIndex) {
        ChessUIGround.instance.AutoUpdate();
        ChessUIGround.instance.HighlightMove(moveFromIndex, moveToIndex);
	}
	...
	public static void SetPositionFromFen (string fen, bool updateBoardUI = true)
	{
		...
		if (enPassantCaptureSquare [0] != '-') {
			initialGameState += (ushort)(DefinitionsGround.fileNames.IndexOf (enPassantCaptureSquare [0]) << 5);
		}
		
		gameStateHistory.Clear ();
		gameStateHistory.Push (initialGameState);

		zobristKey = ZobristKeyGround.GetZobristKey ();
